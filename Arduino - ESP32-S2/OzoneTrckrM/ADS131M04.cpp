...
#define settings SPISettings(12000000, MSBFIRST, SPI_MODE1)
...
void ADS131M04::writeRegisterMasked(uint8_t address, uint16_t value, uint16_t mask)
{
 ...
 //Serial.println(register_contents,HEX);
...
void ADS131M04::sendcmd(uint16_t cmd)
{
  uint16_t data;
...
void ADS131M04::sendcmd16(uint16_t cmd)
{
  uint16_t data;
...
void ADS131M04::begin(uint8_t clk_pin, uint8_t miso_pin, uint8_t mosi_pin, uint8_t cs_pin)
{
  ...
  SPI.begin(ADS131M04_CLK_PIN, ADS131M04_MISO_PIN, ADS131M04_MOSI_PIN, ADS131M04_CS_PIN);
  //SPI.beginTransaction(settings);
...
adcOutput ADS131M04::readADC(void)
{
  ...
  //delayMicroseconds(1);
  digitalWrite(ADS131M04_CS_PIN, HIGH);
	//uint32_t tt2=ESP.getCycleCount();
	//Serial.println((tt2-tt)*4.16,2);
  return res;
}
