...
// ============================================================================
// Write the 2-row metadata header + the CSV column header (no data rows -- the rows are
// streamed in one at a time during the run, see runFileAppendRow). The offset is no longer
// a header line: it is a PHASE-0 data row with flag 0.
static void writeCsvHeader(Print& out, const MeasurementMeta &meta) {
  ...
  out.println(hdr);
  out.println("flag,elevation,mean_ch0,stddev_ch0,mean_ch1,stddev_ch1");
}
...
bool runFileAppendRow(int flag, float el, float m0, float sd0, float m1, float sd1) {
  ...
  char line[96];
  snprintf(line, sizeof(line), "%d,%.4f,%.4f,%.4f,%.4f,%.4f", flag, el, m0, sd0, m1, sd1);
  f.println(line);
  f.close();
...
bool commandLinkBegin() {
  ...
  while (WiFi.status() != WL_CONNECTED) {
    if (millis() - t0 > WIFI_TIMEOUT_MS) {
      Serial.println("CTRL: WiFi timeout -> no remote stop (run ends after LOOPS)");
      return false;
    }
    delay(200);
  }
