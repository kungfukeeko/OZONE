...
void storeMeasurement(float a, float b);

// ---------------- DYNAMIC STORAGE ----------------
float (*measurements)[2] = NULL;
...
void setup() {
  ...
  measurements = (float (*)[2])malloc(MAX_SUBLISTS * sizeof(*measurements));
  if (!measurements) {
...
void loop() {
  ...
  for (int i =0; i<BLOCKS_PER_PHASE; i++) {
    float mean_ch2, stddev_ch2;
    measurement(mean_ch2, stddev_ch2);
    storeMeasurement(mean_ch2, stddev_ch2);
  }
  // ------- PHASE  2 -------
...
*/
  ...
  if (measurement_index >= MAX_SUBLISTS) {
    ...
    for (size_t i = 0; i < MAX_SUBLISTS; i++) {
      ...
      Serial.print(measurements[i][1], 4);
      Serial.println("]");
    }
...
void offsetCalibration() {
  ...
  while (collected < SAMPLES_FOR_CAL) {
    if (drdy_fall) {
      ...
      int32_t delta0 = temp.ch2 - m0;
      m0 += delta0 / (collected + 1);

      collected++;       
    }
  }
  adc1.setChannelOffsetCalibration(2, m0);

  float offset_v2 = m0 / FS * PREF_ADC;

  Serial.print("Reference voltage: ");
  Serial.print(Vref);
  Serial.println(" mV");
  Serial.print("Offset Calibration: ");
  Serial.print(offset_v2);
  Serial.println(" mV   ");
}

// ---------------- READ AND COMPUTE ----------------
void measurement(float &mean_v2, float &stddev_v2) {
  double m0 = 0;
  double s0 = 0;
  int collected = 0;
  ...
  while (collected < SAMPLES_PER_BLOCK) {
    ...
    if (drdy_fall) {
      ...
      adcOutput temp = adc1.readADC();
      Vch2 = temp.ch2 / FS * PREF_ADC;

      double delta0 = Vch2 - m0;
      m0 += delta0 / (collected + 1);
      s0 += delta0 * (Vch2 - m0);
  ...
  mean_v2 = m0;
  float variance2 = s0 / (SAMPLES_PER_BLOCK - 1);
  if (variance2 < 0) variance2 = 0;
  stddev_v2 = sqrt(variance2);

  Serial.print("Treći: " );
  Serial.print(mean_v2);
  Serial.print(" mV   ");
  Serial.println(stddev_v2);
}
...
void storeOffset(float a, float b) {
  if (measurement_index >= MAX_SUBLISTS)
    return;
