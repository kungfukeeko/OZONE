...
bool    hasFix        = false;
bool    hasStoredPos  = false;  // true if NVS has coordinates from a previous GPS fix
...
void setup() {
    ...
    // power up I2C bus — DS3231 needs VCC stable before Wire.begin()
    ...
    prefs.begin("zenith", true);  // read-only
...
void loop() {
    ...
    if (millis() - lastDraw >= 1000UL || lastDraw == 0) {
        ...
        DateTime localNow = DateTime(utcNow.unixtime() + UTC_OFFSET_HOURS * 3600UL);

        // Solar calculation always uses UTC; display shows local time
